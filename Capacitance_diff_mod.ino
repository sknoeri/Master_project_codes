...
void setup() {
  ...
  Wire.write(0xA0);   // writes regsiter 0x07 single conversion enabled
  Wire.write(0x00);   // writes regsiter 0x08 voltage and temp sensordisconected
  Wire.write(0x0B);   // writes regsiter 0x09 EXCA and EXCB pin configured EXCA enable EXCB inverted enabled
  Wire.write(0x01);   // writes regsiter 0x0A sets sample time, puts contineous conversion mode -> important for the reading
  Wire.write(0x9A);   // writes regsiter 0x0B offset calibration on the device for channel A
  Wire.write(0x9F);   // writes regsiter 0x0C offset calibration on the device for channel B
...
uint32_t readCFemtof() // reads the 24 bit capacitance register
{
  ...
  return (8192*((double)C/16777215)); // 8192*C/(2^24-1) ensures value between 0 and 8.192pF/99.2pF. In femto farad.
